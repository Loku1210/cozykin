...
function StudioApp() {
  ...
  const installPack = async () => {
    ...
    const installed = await installAcceptedPack(
      ...
      (error) => window.confirm(`${error instanceof Error ? error.message : String(error)}\n\n是否替换已安装版本？`)
    );
    if (!installed) return;
    notify('角色安装完成。');
